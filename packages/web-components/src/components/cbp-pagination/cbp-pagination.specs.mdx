...
<Meta title="Navigation/Pagination/Specifications" />
...
* For large data sets, a maximum number of pages may be specified for the "pages" dropdown to prevent performance degradation.
...
* The Pagination component shall be reactive to property changes, since it is common for an application to load new data sets or filter data being displayed.
...
* The `records`, `page`, and `pageSize` properties are all reactive - they may be updated dynamically by the application logic.
  * The pages dropdown will be re-generated and next/previous button status reevaluated if either the `records` or `pageSize` property changes.
  * If the data set and/or `records` property is updated, the `page` should also be reset to 1 (by application code) to avoid confusion and/or weird effects.
  * If the `pageSize` property is updated, the page will automatically be set to 1, consistent with the behavior when a user makes the same change.
* Applications with access to large data sets should never return the entire data set to the front-end framework or UI.
  ...
  * Additionally, if the data set results in a large number of pages, you may also limit the number of pages rendered by the dropdown.
    * The dropdown component may experience performance degradation with thousands of pages represented as options.
    * A dropdown with this many items is not usable by humans anyway; it is doubtful that a data set this large will ever be browsed.
* The option to show all records in the "items per page" dropdown should only be used/shown when the data set is known to be limited to a reasonable number of items to view on-screen at once.
* If "All Records" is selected for a data set over 500 records, a warning will be posted to the browser console as a reminder. This is an extreme number and 500 records is likely never a "reasonable number of items to view on-screen at once."
