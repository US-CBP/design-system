...
* The Dropdown options may be specified one of 3 ways: 
  * Slotted as child `cbp-dropdown-item` components.
  * Specified as JSON (stringified or an object) via the `items` property.
  * Asynchronously loaded JSON based on user interaction and a minimum input length.
