...
const Template = ({ label, description, fieldId, name, error, readonly, disabled, input, value, context, sx, items }) => {
  ...
  setTimeout(() => {
    ...
    listbox.addEventListener('updateListboxSuggestions', (e) => {
      console.log('Listbox event received: ', e);
      const { value } = e.detail;
      if (value.length > 0) {
        // filter the JSON natively in JavaScript
        let filteredJSON = Countries.filter( (item) => item.label.toLowerCase().indexOf(value.toLowerCase()) != -1);
        // return the filtered JSON result to the component via the items property
        console.log('filtered JSON: ', filteredJSON);
        listbox.items = filteredJSON;
      }
...
export const ListboxSearch = Template.bind({});
ListboxSearch.storyName="Search with Suggestions"
ListboxSearch.args = {
  label: "Search with Suggestions Listbox",
  description: "This searche cbp-listbox component wraps a native input within the cbp-form-field.",
  input: "search",
  value: "",
};