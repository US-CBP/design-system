...
* Slots are typically reserved for the Shadow DOM, but in our case they are polyfilled by the Stencil library in which the CBP design system web components are built, so they work even without the Shadow DOM.
...
Note: In TypeScript, references to custom elements need to be typed properly to access their properties (and methods) using dot notation. Each web component has its own type in the form of "HTMLCbpTagnameElement".
...
In addition to web component CSS APIs, the design system's <a href="?path=/story/design-tokens--design-tokens">design tokens</a> are exposed via CSS custom properties at the root level.
