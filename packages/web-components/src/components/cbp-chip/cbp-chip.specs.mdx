...
<Meta title="Controls/Chip/Specifications" />
...
* Activating or toggling the chip emits a custom `chipClick` event. The custom event's `details` contain the following keys:
  * `host`: a DOM reference to the chip element activated
  * `name`: the name of the chip, derived from the `name` attribute of the button, if any.
