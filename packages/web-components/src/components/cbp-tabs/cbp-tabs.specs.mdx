...
<Meta title="Content/Tabs/Specifications" />
...
* Tab set:
  ...
  * In the responsive view, the next/previous button controls are *not* keyboard accessible by design because the arrow keys will scroll hidden tabs into view without them; Additionally, the next tab stop should not be in the tab list after selecting a tab.
* Tabs:
  ...
  * Each tab control (button) has an `aria-controls` attribute referencing the corresponding tab panel's `id` (both the `cbp-tab` and `cbp-tab-panel` `name` property must match).
...
* Tab Panels:
  ...
  * Each tab panel has an `aria-describedby` attribute referencing the corresponding tab control's `id`, generated from the panel's "`name`_tab".
...
* A custom event of `tabActivated` is emitted from the tab control (button), which bubbles and may be listened for at a higher level, such as the tabset. 
  * The custom event's `details` contain the following keys:
...
* It is not recommended to set a tab (after the first) as selected by default if it may result in being hidden at smaller screen sizes in responsive mode.
  * This can create usability and accessibility issues.
  * The selected tab will still move into view when tabbed to, however.
