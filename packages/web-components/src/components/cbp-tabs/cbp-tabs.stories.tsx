...
function createTabs(tabs, withIcon, onlyIcon, withBadge, orientation) {
  const html = tabs.map(({ name, label, icon, color, accessibilityText,  selected }) => {
    return `
      ...
      >
        ${withIcon || onlyIcon ? `<cbp-icon ${onlyIcon ? `size="var(--cbp-space-6x)" accessibility-text="${label}"` : ``} name="${icon}"></cbp-icon>` : ''} ${label && !onlyIcon ? label : ''} ${withBadge && !onlyIcon ? `<cbp-badge ${orientation=="vertical" ? `sx='{"margin-left": "auto", "flex-shrink": "0"}'`: ``}>22</cbp-badge>` : ''}
      </cbp-tab>
    `;
  });
  return html.join('');
