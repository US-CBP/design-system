...
* For range sliders, which accept a start and end value, initial values should be set so that the sliders are not placed on top of each other by default. Logical starting values may vary by context.